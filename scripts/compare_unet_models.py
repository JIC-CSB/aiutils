...
@click.command()
@click.argument('model1_uri')
@click.argument('model2_uri')
@click.argument('dataset_uri')
def main(model1_uri, model2_uri, dataset_uri):

    # TODO - shouldn't need input_channels?
    unetmodel1 = TrainedUNet(model1_uri, input_channels=3)
    unetmodel2 = TrainedUNet(model2_uri, input_channels=3)

    ids = ImageDataSet.from_uri(dataset_uri)

    im = ids[7]
    merge1 = unetmodel_and_im_to_mask_vis(unetmodel1, im)
    merge2 = unetmodel_and_im_to_mask_vis(unetmodel2, im)

    import numpy as np
    joined = np.hstack((merge1, merge2))

    joined.view(dbiImage).save("merged.png")
